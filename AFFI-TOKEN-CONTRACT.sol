...
contract AFFITRUM is ERC20, Ownable {
    ...
    function _doAddLp() internal {
        ...
        bool success = false;
        try swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(half,0, pathEth,address(AutoLiquidityTreasury),block.timestamp)
        {
            success = true;
        } catch {}
        if (!success) {
	...
	function _buybackToken() internal swapping { 
      ...
      try swapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmountBefore}(0, path, bonusWallet, block.timestamp) {} catch {}
